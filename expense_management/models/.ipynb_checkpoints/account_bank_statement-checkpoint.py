...
class AccountBankStatement(models.Model):
    ...
    total_credit = fields.Monetary(string="Total Credit",compute="_compute_total_cd_db_")
    total_debit = fields.Monetary(string="Total Debit",compute="_compute_total_cd_db_")
    
    @api.depends('line_ids','line_ids.credit', 'line_ids.debit')
    def _compute_total_cd_db_(self):
        for statement in self:
            total_credit = sum([line.credit for line in statement.line_ids])
            total_debit = sum([line.debit for line in statement.line_ids])
...
class AccountBankStatementLine(models.Model):
    ...
    def _compute_amount(self):
        for line in self:
            if line.amount != line.credit or line.amount != -line.debit:
                if line.debit > 0:
