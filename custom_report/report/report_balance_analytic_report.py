...
class ReportBalanceReportView(models.AbstractModel):
    ...
    def get_lines(self, analytic_id, date_start,date_end):
        ...
        query = """
                SELECT x_aa.code AS code_account, x_aa.name AS name_account, pp.key AS code_project, pp.name AS name_project, x_aan.code AS code_analytic, SUM(x_aml.debit) AS x_debit, SUM(x_aml.credit) AS x_credit, (SUM(x_aml.debit)-SUM(x_aml.credit)) AS solde_debit, (SUM(x_aml.credit)-SUM(x_aml.debit)) AS solde_credit
                ...
                GROUP BY code_project,name_project,code_analytic,code_account,name_account
        """%(date_start,date_end)
