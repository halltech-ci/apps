...
class ReportBalanceReportView(models.AbstractModel):
    ...
    def get_lines(self, analytic_id, date_start,date_end):
        ...
        query = """
                SELECT x_aa.code AS code_account, x_aa.name AS name_account, pp.key AS code_project, pp.name AS name_project, aat.name AS name_tags, SUM(x_aml.debit) AS x_debit, SUM(x_aml.credit) AS x_credit, (SUM(x_aml.debit)-SUM(x_aml.credit)) AS solde_debit, (SUM(x_aml.credit)-SUM(x_aml.debit)) AS solde_credit

                FROM account_move_line AS x_aml
                INNER JOIN account_account AS x_aa ON  x_aa.id = x_aml.account_id_cle
                ...
                INNER JOIN account_analytic_tag AS aat ON aat.account_id = x_aa.id
                WHERE 
                ...
                GROUP BY code_project,name_project,name_tags,code_account,name_account
        """%(date_start,date_end)
